...
T3 Code is a minimal GUI for coding agents. A Node WebSocket server wraps provider CLIs (Codex, Claude Code, Cursor, Grok, OpenCode, Pi) and serves web, desktop, and mobile clients.
...
- **Providers.** Codex, Claude, Cursor, Grok, OpenCode, and Pi each have an adapter. Provider-shaped features need a decision per adapter, even if the decision is "not supported here".
